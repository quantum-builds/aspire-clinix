...
export default async function AppointmentRequestFormWrapper({
  ...
  const response: Response<TAppointmentRequest> = await getAppointmentRequest(id);

  let dentists: TDentist[] = [];
  let isLoadingDentists = false;

  if (practiceId.trim().length > 0) {
    isLoadingDentists = true;
    const dentistResponse: Response<TDentistPractice[]> = await getDentistPractice({
      practiceId,
      status: PracticeApprovalStatus.APPROVED,
    });
    isLoadingDentists = false;

    dentists = dentistResponse.data?.map((dp) => dp.dentist) ?? [];
  }
  ...
  return (
    <BookAppointmentForm
      ...
      dentists={dentists}
      appointmentRequest={response.data}
      dentistLoading={isLoadingDentists}
    />
  );
}
